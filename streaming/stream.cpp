...
int main(int argc, char* argv[])
{
    ...
    // this the parser eroor
    catch (ParserError& x) {
      cout << x.what() << endl;
      return 1;
    }
    // the other errors with the parser
    ...
    while(option != 0) {
        ...
        catch(UserNotLoggedInError& x) {
          cout << no_user_except_msg << endl;
        }
        
        // if the rating is too high
        catch (RatingLimitError& x) {
          cout <<rating_limit_except_msg << endl;
        }
        // the review needs to be in the rang 0-5
        catch (ReviewRangeError& x) {
